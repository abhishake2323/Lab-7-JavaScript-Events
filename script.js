...
for (let index = 0; index < formInputs.length; index++) {
    ...
    formInputs[index].addEventListener('focusout', function (event) {
        document.getElementById('focusStat').innerHTML = `Focussed out the <b> ${formInputs[index].attributes['data-id'].value} </b> field from the form`
    })
...
document.getElementById('buttonContainer').addEventListener('click', function (event) {
    let clickedElement = event.target; // Check if a button was clicked 
    if (clickedElement.tagName === 'BUTTON') { const message = clickedElement.getAttribute('data-cont'); document.getElementById('BtnMessage').textContent = message; }
});
